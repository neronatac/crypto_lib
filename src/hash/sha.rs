...
fn sha256_expansion(w: &mut [u32; 64]) {
    let sigma0 = |x: u32| -> u32 { x.rotate_right(7) ^ x.rotate_right(18) ^ x.shr(3) };
    let sigma1 = |x: u32| -> u32 { x.rotate_right(17) ^ x.rotate_right(19) ^ x.shr(10) };
    for t in 16..64 {
        w[t] = sigma1(w[t - 2])
            .wrapping_add(w[t - 7])
...
fn sha512_expansion(w: &mut [u64; 80]) {
    let sigma0 = |x: u64| -> u64 { x.rotate_right(1) ^ x.rotate_right(8) ^ x.shr(7) };
    let sigma1 = |x: u64| -> u64 { x.rotate_right(19) ^ x.rotate_right(61) ^ x.shr(6) };
    for t in 16..80 {
        w[t] = sigma1(w[t - 2])
            .wrapping_add(w[t - 7])
            .wrapping_add(sigma0(w[t - 15]))
...
fn sha256_round(round: usize, wv: &mut [u32; 8], w: &[u32; 64]) {
    ...
    let sigma0 = |x: u32| -> u32 { x.rotate_right(2) ^ x.rotate_right(13) ^ x.rotate_right(22) };
    let sigma1 = |x: u32| -> u32 { x.rotate_right(6) ^ x.rotate_right(11) ^ x.rotate_right(25) };
    let k: [u32; 64] = [
    ...
    let t2 = sigma0(wv[0]).wrapping_add(maj(wv[0], wv[1], wv[2]));
...
fn sha512_round(round: usize, wv: &mut [u64; 8], w: &[u64; 80]) {
    ...
    let sigma0 = |x: u64| -> u64 { x.rotate_right(28) ^ x.rotate_right(34) ^ x.rotate_right(39) };
    let sigma1 = |x: u64| -> u64 { x.rotate_right(14) ^ x.rotate_right(18) ^ x.rotate_right(41) };
    let k: [u64; 80] = [
        0x428a2f98d728ae22,
        0x7137449123ef65cd,
        0xb5c0fbcfec4d3b2f,
...
macro_rules! create_sha {
    ...
    (@internal
        ...
        paste! {
            ...
            impl Hash<$digest_size> for [< SHA $sha_number>] {
...
mod tests_sha0 {
    ...
    fn test_big() {
        ...
        assert_eq!(
            res,
            [
                0x4a, 0xa2, 0x9d, 0x14, 0xd1, 0x71, 0x52, 0x2e, 0xce, 0x47, 0xbe, 0xe8, 0x95, 0x7e,
                0x35, 0xa4, 0x1f, 0x3e, 0x9c, 0xff
            ]
        );
    }
    ...
    fn test_bigbig() {
        ...
        assert_eq!(
            res,
            [
                0xbc, 0xbe, 0xd6, 0xdd, 0x21, 0x56, 0xbf, 0x13, 0x5d, 0x42, 0x19, 0x81, 0x24, 0x9a,
                0xcd, 0xd3, 0xc9, 0x73, 0x4d, 0x1f
            ]
        );
    }
}
...
mod tests_sha1 {
    ...
    fn test_big() {
        ...
        assert_eq!(
            res,
            [
                0x50, 0xab, 0xf5, 0x70, 0x6a, 0x15, 0x09, 0x90, 0xa0, 0x8b, 0x2c, 0x5e, 0xa4, 0x0f,
                0xa0, 0xe5, 0x85, 0x55, 0x47, 0x32
            ]
        );
    }
    ...
    fn test_bigbig() {
        ...
        assert_eq!(
            res,
            [
                0x9a, 0xb9, 0xa9, 0x73, 0xce, 0x40, 0x3f, 0x31, 0xfe, 0x04, 0x15, 0x42, 0x1d, 0xc3,
                0x60, 0x8d, 0xf7, 0x24, 0x61, 0x3c
            ]
        );
    }
}
...
mod tests_sha256 {
    ...
    fn test_big() {
        ...
        assert_eq!(
            res,
            [
                0xf3, 0x71, 0xbc, 0x4a, 0x31, 0x1f, 0x2b, 0x00, 0x9e, 0xef, 0x95, 0x2d, 0xd8, 0x3c,
                0xa8, 0x0e, 0x2b, 0x60, 0x02, 0x6c, 0x8e, 0x93, 0x55, 0x92, 0xd0, 0xf9, 0xc3, 0x08,
                0x45, 0x3c, 0x81, 0x3e
            ]
        );
    }
    ...
    fn test_bigbig() {
        ...
        assert_eq!(
            res,
            [
                0xb0, 0x36, 0x49, 0x6b, 0x6c, 0x63, 0x0d, 0xe4, 0xac, 0x9a, 0xc3, 0x33, 0x77, 0xc7,
                0x0d, 0x58, 0xca, 0x04, 0xb9, 0xac, 0x4d, 0x3c, 0x78, 0xd2, 0xeb, 0x25, 0xe6, 0x84,
                0x6f, 0xfd, 0x65, 0x08
            ]
        );
    }
...
mod tests_sha512 {
    ...
    fn test_big() {
        ...
        assert_eq!(
            res,
            [
                0x72, 0xec, 0x1e, 0xf1, 0x12, 0x4a, 0x45, 0xb0, 0x47, 0xe8, 0xb7, 0xc7, 0x5a, 0x93,
                0x21, 0x95, 0x13, 0x5b, 0xb6, 0x1d, 0xe2, 0x4e, 0xc0, 0xd1, 0x91, 0x40, 0x42, 0x24,
                0x6e, 0x0a, 0xec, 0x3a, 0x23, 0x54, 0xe0, 0x93, 0xd7, 0x6f, 0x30, 0x48, 0xb4, 0x56,
                0x76, 0x43, 0x46, 0x90, 0x0c, 0xb1, 0x30, 0xd2, 0xa4, 0xfd, 0x5d, 0xd1, 0x6a, 0xbb,
    ...
    fn test_bigbig() {
        ...
        assert_eq!(
            res,
            [
                0x33, 0xf8, 0x90, 0x1b, 0x05, 0x3e, 0x4c, 0xc6, 0x77, 0xd3, 0xcb, 0x41, 0x22, 0xd9,
                0x6a, 0xd9, 0xb9, 0x6b, 0x13, 0xbf, 0x76, 0x19, 0x4c, 0xf9, 0x62, 0x48, 0x8b, 0xb4,
                0xde, 0x49, 0x98, 0xa7, 0x14, 0x55, 0xcb, 0x31, 0x58, 0x2d, 0xb5, 0x27, 0xad, 0xf7,
                0x7a, 0x48, 0x5b, 0x81, 0xcf, 0x5b, 0x72, 0x2a, 0x5e, 0x86, 0x38, 0xeb, 0x6b, 0xe4,
                0x87, 0x40, 0x0f, 0x3a, 0xec, 0x00, 0x6e, 0x7c
            ]
        );
    }
}
...
mod tests_sha384 {
    ...
    fn test_big() {
        ...
        assert_eq!(
            res,
            [
                0xb1, 0x29, 0x32, 0xb0, 0x62, 0x7d, 0x1c, 0x06, 0x09, 0x42, 0xf5, 0x44, 0x77, 0x64,
                0x15, 0x56, 0x55, 0xbd, 0x4d, 0xa0, 0xc9, 0xaf, 0xa6, 0xdd, 0x9b, 0x9e, 0xf5, 0x31,
                0x29, 0xaf, 0x1b, 0x8f, 0xb0, 0x19, 0x59, 0x96, 0xd2, 0xde, 0x9c, 0xa0, 0xdf, 0x9d,
                0x82, 0x1f, 0xfe, 0xe6, 0x70, 0x26
            ]
        );
    }
    ...
    fn test_bigbig() {
        ...
        assert_eq!(
            res,
            [
                0x3c, 0x90, 0xd9, 0xea, 0x7b, 0x4e, 0xb7, 0x19, 0xc2, 0x66, 0x05, 0x02, 0xce, 0x20,
                0x29, 0x49, 0x93, 0x8d, 0xdd, 0x65, 0x38, 0xcc, 0x93, 0xbd, 0x4f, 0x98, 0xad, 0x7e,
                0x60, 0xa9, 0x26, 0xed, 0x7c, 0xbc, 0xb7, 0x3e, 0x33, 0x0b, 0x1a, 0x72, 0xab, 0x78,
                0xc7, 0x21, 0xda, 0x63, 0x46, 0xd4
            ]
        );
    }
}
