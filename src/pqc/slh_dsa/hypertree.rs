    ...
    [u8; N]: Pod,
{
    ...
    // import method is defined here because HypertreeSignature does not know its own length (param D)
    pub fn signature_from_bytes(&self, bytes: &[u8]) -> HypertreeSignature<N> {
        let mut sigs_xmss = Vec::<XMSSSignature<N>>::with_capacity(D);

        let mut idx = 0;
        for _ in 0..D {
            let elem = self.xmss.signature_from_bytes(&bytes[idx..]);
            idx += elem.len();
            sigs_xmss.push(elem);
        }
