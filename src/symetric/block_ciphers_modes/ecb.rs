...
mod tests_ecb {
    ...
    fn ecb_encrypt_bad_length() {
        ...
        let res = <ECB as BlockChaining<AES128>>::cipher(&plain, &mut ciphertext, &key);
        assert!(res.is_err());
    }
    ...
    fn ecb_decrypt_2_blocks() {
        ...
        <ECB as BlockChaining<AES128>>::decipher(&mut plain, &ciphertext, &key).expect("Error during ECB_AES128 execution");
    ...
    fn ecb_decrypt_bad_length() {
        ...
        let res = <ECB as BlockChaining<AES128>>::decipher(&mut plain, &ciphertext, &key);
        assert!(res.is_err());
    }
